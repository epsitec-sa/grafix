...
void
AggFontPixelCacheGlyphXY(AggBuffer* buffer, agg::font_face* face,
						 ...
						 double r, double g, double b, double a)
{
	...
	agg::int32u i_size_x = static_cast<agg::int32u> (scale_x * 16);
	agg::int32u i_size_y = static_cast<agg::int32u> (scale_y * 16);
	agg::int32u i_size_s = (i_size_x & 0xffff) | ((i_size_y & 0xffff) << 16);
	...
	agg::font_face::cache_record::pix_data_record*  data = info ? face->FindPixData (info, i_size_s, i_xs, i_ys) : 0;
	  ...
	  && (data->pixels == 0) )
	{
		...
		agg::font_path_provider font_path = agg::font_path_provider (face, glyph, info, ox, oy, scale_x, scale_y);
...
#if defined(EMULATE_CLEARTYPE_X3)
			agg::font_path_provider font_path = agg::font_path_provider (face, glyph, info, data->ox / 256.0, data->oy / 256.0, i_size_x * 3 / 16.0, i_size_y / 16.0);
			agg::conv_curve<agg::font_path_provider> curve (font_path);
...
#else
			agg::font_path_provider font_path = agg::font_path_provider (face, glyph, info, data->ox / 256.0, data->oy / 256.0, i_size_x / 16.0, i_size_y / 16.0);
			agg::conv_curve<agg::font_path_provider> curve (font_path);
			agg::rasterizer_scanline_aa<> rasterizer;
			agg::rendering_buffer buffer;
